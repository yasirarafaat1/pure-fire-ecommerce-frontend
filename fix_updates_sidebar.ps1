...
$pattern = '(?s)\{updatesVisible && \(.*?\)\}\s*\n\s*\n\s*<div className="grid gap-5 md:grid-cols-\[1\.2fr_0\.8fr\] items-start">'
$newBlock = @'
{updatesVisible && (
...
if ($txt -notmatch $pattern) { Write-Error 'Sidebar block not found.'; exit 1 }
$txt = [regex]::Replace($txt, $pattern, $newBlock, 1)
